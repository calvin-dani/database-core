...
RC RelationManager::createCatalog()
{
    ...
    */
    ...
    if(rbfm->createFile("Tables")==0 && rbfm->createFile("Columns")==0)
    {
        ...
        RID rid;
        void *data = malloc(TABLES_CATALOG_RECORD_SIZE); 

        //Tables catalog part        
        rc = rbfm->openFile("Tables", fileHandle);
        if(rc==-1)
            return -1;
        assert(rc==0 && "open Tables");
...
RC RelationManager::deleteCatalog()
{
    ...
    if(rc==-1)
        return -1;
	assert(rc == 0 && "Tables catalog deleted");
    rc = rbfm->destroyFile("Columns");
	if(rc==-1)
        return -1;
    assert(rc == 0 && "Columns catalog file deleted");
	return 0;    
}
...
RC RelationManager::createTable(const string &tableName, const vector<Attribute> &attrs)
{
    ...
    RID rid;
    void *data = malloc (1 + 4); //reserve space for null byte + 4 bytes (int size).
    ...
    //vector<Attribute> TablesDescriptor;
    //createTablesDescriptor(TablesDescriptor);
    void *tuple = malloc(TABLES_CATALOG_RECORD_SIZE);
...
RC RelationManager::deleteTable(const string &tableName)
{
    ...
    void *value = malloc(4+50); //reserve space for table-name:varchar(50)
